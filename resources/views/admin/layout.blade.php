...
(() => {
    ...
    document.querySelectorAll('[data-live-search]').forEach(form => {
        ...
        const loadResults = async url => {
            ...
            controller?.abort(); controller = new AbortController(); results.classList.add('is-loading'); setButtonLoading(true);
            ...
            finally { if (currentRequest === requestId) { results.classList.remove('is-loading'); setButtonLoading(false); } }
        };
        ...
        input.addEventListener('input',()=>{clearTimeout(timer);setButtonLoading(true);timer=setTimeout(search,300)});
